...
def extract_zip_files(downloaded_path: Path) -> int:
    ...
    # O dataset Instacart pode ser entregue como arquivos .zip.
...
def validate_dataset_files() -> None:
    ...
    """
    available_files = {
        file_path.name for file_path in RAW_DATA_DIR.glob("*.csv")
    }
...
def download_dataset(force_download: bool = False) -> Path:
    """
    Baixa o dataset Instacart usando o cache local do Kaggle quando possível.
